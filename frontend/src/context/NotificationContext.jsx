...
export const NotificationProvider = ({ children }) => {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    if (!endpoint) return;
    try {
      const res = await fetch(`http://https://dwo-final.onrender.com${endpoint}`, { headers: { 'x-user': JSON.stringify(user) } });
      if (res.ok) {
  ...
  const markAsRead = useCallback(async (id) => {
    ...
    const user = JSON.parse(localStorage.getItem('user'));
    try {
      await fetch(`http://https://dwo-final.onrender.com/api/patient/notifications/${id}/mark-read`, {
        method: 'PUT',
  ...
  const markAllRead = useCallback(async () => {
    ...
    const user = JSON.parse(localStorage.getItem('user'));
    try {
      await fetch(`http://https://dwo-final.onrender.com/api/patient/notifications/mark-read`, {
        method: 'PUT',
        headers: { 'x-user': JSON.stringify(user) }
      });
    } catch (err) { console.error(err); }
